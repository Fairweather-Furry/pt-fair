...
function peppibot_factory_pepp_tom()
  return(
    has("op_pepp") and ((sjump() and (climb() or upper())) or (has("op_diff_exp") and (sjump() or (climb() and (grab() or upper())))))
  )
end

function peppibot_factory_pepp_com()
  return(
    box() and pepp_slam() and ((sjump() and (climb() or upper())) or (has("op_diff_exp") and (sjump() or (climb() and (grab() or upper())))))
  )
end
...
function peppibot_factory_pepp_srank()
  return(
    pepp_slam() and box() and ((grab() and sjump() and (climb() or upper())) or (has("op_diff_exp") and (climb_or_jump() and (grab() or upper()))))
  )
end

function peppibot_factory_pepp_pum()
  return(
    has("op_pepp") and box() and ((sjump() and (climb() or upper())) or (has("op_diff_exp") and climb() and (grab() or upper())))
  )
end

function peppibot_factory_noise_com()
  return(
    box() and ((sjump() and (noise_slam() or torn() or crush())) or (has("op_diff_exp") and (crush() or ((sjump() or upper()) and (bounce() or torn())))))
  )
end

function peppibot_factory_noise_srank()
  return(
    box() and ((grab() and sjump() and (noise_slam() or torn() or crush())) or (has("op_diff_exp") and ((crush() and (grab() or upper())) or (upper() and (bounce() or torn())) or (sjump() and bounce() and grab()))))
  )
end

function oh_shit_pepp_che()
  return(
    pepp_slam() and (climb() or sjump() or upper())
  )
end
...
function oh_shit_noise_com()
  return(
    sticky() and (((noise_slam() or torn() or crush()) and (bounce_or_jump() or upper())) or (has("op_diff_exp") and (bounce() or crush())))
  )
end
...
function pizzascare_pepp_tom()
  return(
    pepp_slam() and (climb() or (sjump() and has("op_diff_exp")))
  )
end
...
function pizzascare_noise_che()
  return(
    (crush() or ((noise_slam() or torn()) and (bounce_or_jump() or upper()))) or (has("op_diff_exp") and bounce())
  )
end

function pizzascare_noise_tom()
  return(
    ((noise_slam() or torn() or crush()) and (bounce_or_jump())) or (has("op_diff_exp") and (crush() or bounce() or (noise_upper() and (slam() or torn()))))
  )
end
...
function war_noise_mus()
  return(
    shotgun() and (bounce_or_jump() or (has("op_diff_exp") and (crush() or noise_slam())))
  )
end

function war_noise_sau()
  return(
    shotgun() and rocket() and (((slam() or crush() or torn()) and bounce_or_jump()) or (has("op_diff_exp") and (crush() or bounce_or_jump() or noise_slam())))
  )
end
...
function tricky_treat_pepp_pum3()
  return(
    has("op_pepp") and ghost() and (upper() or climb_or_jump())
  )
end

function tricky_treat_noise_pum3()
  return(
    has("op_noise") and ghost() and (upper() or bounce_or_jump() or crush())
  )
end

function tricky_treat_noise_pum4()
  return(
    ghost() and ball() and (bounce_or_jump() or (has("op_diff_exp") and crush()))
  )
end
...
function s_p_ranked(slot)
  ...
  elseif level == 14 then
    return((peppibot_factory_pepp_srank() or peppibot_factory_noise_srank()) and lap2())
